...
TSHARK_OPT = ["-t e",
            "-T fields",
            "-E separator=,",
            "-e ip.src -e ip.dst -e ip.proto -e tcp.len -e udp.length -e eth.src -e eth.dst",
            "-e tcp.srcport -e tcp.dstport -e udp.srcport -e udp.dstport",
            ]

PACKET_COLUMNS =  ['ip_src', 'ip_dst', 'proto','len_tcp','len_udp', 'mac_src', 'mac_dst',
...
def executeCommand(command, outfilename, rotulos, salveFilter,rotul):
    ...
    #df = pd.read_csv(outfilename, names = PACKET_COLUMNS)
    #df.columns = PACKET_COLUMNS

    df.fillna(0, inplace = True)
    #df['len'] = df['len_udp'] + df['len_tcp'] # junta colunas de tamanho pacote
